...
<?php
session_start();
include_once "engine/config.php"; // Ensure database connection is included

if (isset($_SESSION['lawyer_id']) || isset($_SESSION['firm_id'])  || isset($_SESSION['id'])) {

    $host = $_SERVER['HTTP_HOST'];
    $hostParts = explode('.', $host);

    // Ensure we are handling a valid subdomain
    if (count($hostParts) > 2) {
        $username = filter_var($hostParts[0], FILTER_SANITIZE_STRING);

        // Prevent both business and service provider from being active
        if (isset($_SESSION['lawyer_id']) && isset($_SESSION['firm_id']) && isset($_SESSION['id'])) {
        ...
        if ($user) {
            ...
            // Display the user profile dynamically
            if (isset($_SESSION['firm_id'])) {
                echo "<h1>Welcome to {$user['firm_name']}'s profile!</h1>";
                echo "<div class='user-content'>{$user['firm_details']}</div>";

            } elseif (isset($_SESSION['lawyer_id'])) {
                
                echo "<h1>Welcome to {$user['lawyer_name']}'s profile!</h1>";
                echo "<div class='user-content'>{$user['lawyer_details']}</div>";
            }

            include_once "footer.php";
        } else {
            // Subdomain does not match any user
            header("HTTP/1.0 404 Not Found");
            include_once "templates/404.php";
        }
    } else {
        // If not a subdomain, load main website
        include_once "elegal.ng";
    }
}
?>
