<?php 
     ...
     $lawyer_qualification =  isset($row['lawyer_qualification']) && !empty($row['lawyer_qualification']) ? $conn->real_escape_string($row['lawyer_qualification']) : " ";
     $lawyer_firm = isset($row['lawyer_firm']) && !empty($row['lawyer_firm']) ? $conn->real_escape_string($row['lawyer_firm']) : "not available ";
     $lawyer_role =  isset($row['lawyer_role']) && !empty($row['lawyer_role']) ?  $conn->real_escape_string($row['lawyer_role']) : "not available";
     $lemployment_status = isset($row['employment_status']) && !empty($row['employment_status']) ?  $conn->real_escape_string($row['employment_status']) : "not employed ";
