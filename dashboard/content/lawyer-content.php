 <?php 
         ...
         if ($row = mysqli_fetch_assoc($result)) {
             $user_img = $row['lawyer_img'] ?? "https://placehold.co/600x400";
             $date = $row['created_at'] ?? " ";
             ...
             $lawyer_qualification = $row['lawyer_qualification'] ?? " ";
             $lawyer_firm = $row['lawyer_firm'] ?? "not available ";
             $lawyer_role = $row['lawyer_role'] ?? "not available";
             $employment_status = $row['employment_status'] ?? "not employed ";
