...
<table class='table table-responsive table-striped table-hover w-100 text-sm px-2'>
    ...
    <tbody>
        ...
        $query = "
            ...
            WHERE case_status = 'concluded' ORDER BY clients_on_board.date_created DESC
        ";
        ...
        <tr>
            ...
            <td class='text-capitalize'><?= htmlspecialchars($row['case_status'] ?? " not available"); ?></td>
            <td class='text-'><?= htmlspecialchars($row['paid'] ?? " not available")."%"; ?></td>
